...
export function CallToActionSection() {
  return (
    <section className="py-20 bg-cream-50 dark:bg-cream-900 relative overflow-hidden">
      {/* Background pattern */}
      <div className="absolute inset-0 opacity-10 dark:opacity-20">
        <svg className="h-full w-full" viewBox="0 0 800 800">
          <defs>
            <pattern id="cta-pattern" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M.5 40V.5H40" fill="none" stroke="currentColor" strokeOpacity="0.3" />
            </pattern>
          </defs>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <motion.div
          ...
          className="bg-elevated dark:bg-white/10 border border-default dark:border-border-dark rounded-3xl shadow-lg overflow-hidden"
        >
          <div className="grid lg:grid-cols-2 gap-12 items-center relative">
            {/* Floating circles */}
            <motion.div
              className="absolute -top-20 right-20 w-40 h-40 bg-primary-light dark:bg-primary/20 backdrop-blur-xl rounded-full"
              animate={{ scale: [1, 1.05, 1], rotate: [0, 5, 0] }}
              transition={{ duration: 8, repeat: Infinity }}
            />
            <motion.div
              className="absolute -bottom-20 -left-20 w-40 h-40 bg-primary-lighter dark:bg-primary/20 backdrop-blur-xl rounded-full"
